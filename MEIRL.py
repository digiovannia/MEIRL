...
def state_index(tup, D):
    return D*tup[0] + tup[1]


def multi_state_index(states, D):
    return D*states[:,0]+states[:,1]


def transition(state_space, action_space, D, MOVE_NOISE):
    '''
            ...
            for x2 in range(D):
                for y2 in range(D):
                    if ((x2 == x1 + action[0]) and
                      (y2 == y1 + action[1])):
                        TP[s,a,state_index((x2,y2),D)] += 1 - MOVE_NOISE
                    if manh_dist((x2, y2), state_space[s]) == 1:
                        TP[s,a,state_index((x2,y2),D)] += MOVE_NOISE/4
                    if ((x2, y2) == state_space[s]).all():
                        count = (x2 == 0 or x2 == D-1) + (y2 == 0 or y2 == D-1)
                        TP[s,a,state_index((x2,y2),D)] += count*MOVE_NOISE/4
                        if ((x1 + action[0] < 0) or
                          (x1 + action[0] >= D) or
                          (y1 + action[1] < 0) or
                          (y1 + action[1] >= D)):
...
def myo_policy(beta, s, TP, rewards, D):
    ...
    expected reward)
    '''
    expect_rew = TP[state_index(s,D)].dot(np.ravel(rewards))
...
def make_data(alpha, sigsq, rewards, N, Ti, state_space, action_space,
    ...
    trajectories = [[synthetic_traj(rewards, alpha, sigsq, i, Ti, state_space,
      action_space, TP, MOVE_NOISE, ETA_COEF, D, INTERCEPT_ETA,
      Q) for i in range(m)] for _ in range(N)]
    return trajectories 
...
def alpha_grad_EM(gZ_alpha, E_all, R_all):
    result = -gZ_alpha + np.einsum('ijk,ik->ij', E_all, R_all)[:,None,:]
    return np.mean(result, axis=1)


def sigsq_grad_EM(gZ_sigsq, normals, Ti, sigsq, gnorm, denom, R_all, gvec):
    q_grad = -Ti/(2*denom)
...
def results_var_hyper(id_num, param, par_vals, seed, test_data='myo',
    ...
    for par in par_vals:
        ...
        TP = transition(state_space, action_space, D, MOVE_NOISE)
...
def summary():
    ...
    dfdict = {'ETA_COEF': [], 'N': [], 'ex_sigsqs': [], 'SEED_NUM': [],
              ...
              'sd MEIRL_unif_tot': [], 'mean MEIRL_EM_tot': [],
              'sd MEIRL_EM_tot': [], 'mean random_tot': [],
              'sd random_tot': []}
                ...
                for line in f:
                    if '=' in line:                            
                        ...
                        if key in ['mean AR_AEVB_tot', 'sd AR_AEVB_tot']:
                            key = key.replace('AR_AEVB', 'MEIRL_EM')
...
def average_within_algo(df, filt_dict=False, save=False):
    ...
    performance =['true_tot', 'mean MEIRL_det_tot', 'mean MEIRL_unif_tot',
                  'mean MEIRL_EM_tot', 'mean random_tot']
    names = ['True', 'MEIRL-Det', 'MEIRL-Unif', 'MEIRL-EM', 'Random']
    colors = ['b', 'r', 'g', 'k', 'm']
    x = np.arange(5)*0.5
...
def compare_plots(df, filt_dict_1, filt_dict_2, filt_text_1, filt_text_2,
    ...
    mod_d1 = data_1.copy()
    mod_d2 = data_2.copy()
    mod_d1[['sd MEIRL_det_tot', 'sd MEIRL_unif_tot', 'sd MEIRL_EM_tot',
      'sd random_tot']] = mod_d1[['sd MEIRL_det_tot', 'sd MEIRL_unif_tot',
      'sd MEIRL_EM_tot', 'sd random_tot']]**2
    mod_d2[['sd MEIRL_det_tot', 'sd MEIRL_unif_tot', 'sd MEIRL_EM_tot',
